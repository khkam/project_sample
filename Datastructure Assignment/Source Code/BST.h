...
struct BST {
	...
	void deepest(BTNode *node, int level);
	bool deepestNodes();

	//Question C
	bool display(int order, int source);
	void inOrderPrintInFile(ostream &out);
	void inOrderPrintInFile2(BTNode *cur, ostream &out);
	void inOrderPrintInFileDescendingOrder(ostream &out);
	void inOrderPrintInFileDescendingOrder2(BTNode *cur, ostream &out);

	//Question D
	bool CloneSubtree(BST t1, type item);
	bool findNode(BST t1, type item, bool &found);
	void findNode2(BTNode *cur, type item, bool &found);
	void findNode3(BTNode *cur);

	//Question E
	bool printAncestor(type item);
	bool printAncestor_2(type item, BTNode *node);

	//Question F
	bool printSpiral();
	bool printLevelFromLeftToRight(BTNode* node, int level);
	bool printLevelFromRightToLeft(BTNode* node, int level);
};
#endif